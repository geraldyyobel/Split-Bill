  ...
  const handleImageFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (!ctx) {
          // Fallback if canvas is not supported/failing
          parseReceipt(event.target?.result as string);
          return;
        }
        ctx.drawImage(img, 0, 0, width, height);

        // Compress image to JPEG at 0.75 quality to stay well under Vercel payload limits and timeout limits
        const compressedBase64 = canvas.toDataURL("image/jpeg", 0.75);
        parseReceipt(compressedBase64);
      };
      img.onerror = () => {
